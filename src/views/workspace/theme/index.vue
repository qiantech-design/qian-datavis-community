            ...
            <el-table :data="state.listData" height="100%" style="width: 100%">
              ...
              <el-table-column prop="list" label="颜色预览" min-width="200">
                <template #default="scope">
                  <span v-for="(item, index) in scope.row.colorList" :key="index"
                    style="display: inline-block;margin-left: 10px;"
                    :style="{ width: '15px', height: '15px', background: item.color }"></span>
                </template>
              </el-table-column>
              <el-table-column fixed="right" label="操作" min-width="120">
                <template #default="scope">
                  <el-button link type="primary" size="small"
                    @click="handleCommandList('edit', scope.row)">编辑</el-button>
                  <el-button link type="primary" size="small" @click="handleCommandList('delete', scope.row)">
                    删除
                  </el-button>
                </template>
              </el-table-column>
...
const getFolders = async ({ parent_id, type }: any) => {
  const params = { parent_id, type, sortBy: 'created_at', sortOrder: 'asc' }
