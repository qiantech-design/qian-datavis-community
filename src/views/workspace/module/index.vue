...
const getFolders = async ({ parent_id, type, name }: any) => {
  const params = { parent_id, type, sortBy: 'sort', sortOrder: 'asc', name }
...
const initGetClassData = async () => {
  state.classData = await getFolders({ type: state.config.type })
  if (state.classData.length) {
    handleChangeClass(state.classData[0])
  }
}
...
const initGetListData = async () => {
  state.listData = []
  const [err, res] = await apiHook.getResource({
